#---------------------------------------------------------------------------------------------
# jy_btcchina.py -- a console tool to do buy/sell for okcoin
#
...
# buy_market
#-------------------------------   
def buy_market(access_key,secret_key,amount):
    bc = PyOKCoin.OKCoin(access_key,secret_key)
    i = 3
    ret_code = -1
    while i > 0 and ret_code < 0:
        i = i -1
        ret_code,resp = bc.buy_market2(amount)
        if ret_code == 0:
...
# sell_market
#-------------------------------   
def sell_market(access_key,secret_key,amount):
    bc = PyOKCoin.OKCoin(access_key,secret_key)
    i = 3
    ret_code = -1
    while i > 0 and ret_code < 0:
        i = i -1
        ret_code,resp = bc.sell_market2(amount)
        if ret_code == 0:
            return ret_code,resp
        time.sleep(1)
    return -1,''
...
def get_btc_cny(access_key,secret_key):
    ...
    while i > 0 and ret_code < 0:
        i = i -1
        ret_code, q, b = bc.get_account_info()
        if ret_code == 0:
            return ret_code,float(b),float(q)
        time.sleep(3)
    return -1,0,0
...
while True:
    ...
    if ch == 'a' or ch == 'b':
        ...
        ret,resp = buy_market(access_key,secret_key,float(BUY_SELL_UNIT))
        print 'ret:',ret,' resp:',resp
        
        writefile2('do.txt','buy ' + BUY_SELL_UNIT + '->' + str(resp) + '\r\n')
    elif ch == 'q' or ch == 's':
        ret,resp = sell_market(access_key,secret_key,float(BUY_SELL_UNIT))
        print 'ret:',ret,' resp:',resp
        writefile2('do.txt','sell ' + BUY_SELL_UNIT + '->'  + str(resp) + '\r\n')

    elif ch == 'e' or ch == 'd' or ch == ' ':
        okcoin = PyOKCoin.OKCoin(access_key,secret_key)
        ret_code, v = okcoin.get_lastprice()
        if (ret_code == 0):
            
            print 'ret:',ret_code,' price:',v
            ret,b,q = get_btc_cny(access_key,secret_key)
            if (ret == 0):
                s2 = ' btc:' + str(b) + ' cny:' + str(q) + ' btc+cny:' + str(b*float(v) + q)
                print s2
